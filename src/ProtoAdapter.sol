...
//         address _user,
//         uint256 _amount
//     ) internal view returns (Account memory user) {
...
//             WAD) / (SECONDS_PER_YEAR * DECIMALS_ADJUSTMENT);
...
//         uint256 maxStakeDebt = (stakedBalance * maxLockDuration * WAD) /
//             (SECONDS_PER_YEAR * DECIMALS_ADJUSTMENT);
...
//         uint256 availableToWithdraw = portalEnergy >= maxStakeDebt
//             ? stakedBalance
...
//         /// @dev Update the user's maxStakeDebt based on added stake amount and current maxLockDuration
//         uint256 maxStakeDebt = (_amount * maxLockDuration * WAD) /
//             (SECONDS_PER_YEAR * DECIMALS_ADJUSTMENT);
...
//         Account memory account = accounts[_user].isExist
//             ? _updateAccount(_user, _amount)
//             : _createAccount(_amount);
...
//     function unstake(
//         uint256 _amount
//     ) external nonReentrant existingAccount(msg.sender) {
//         if (_amount == 0) revert ErrorsLib.InvalidInput();

//         Account memory account = _updateAccount(msg.sender, 0);
//         if (_amount > account.availableToWithdraw)
...
//             (stakedBalance * maxLockDuration * WAD) /
//             (SECONDS_PER_YEAR * DECIMALS_ADJUSTMENT);
//         uint256 portalEnergy = account.portalEnergy -=
//             (_amount * maxLockDuration * WAD) /
//             (SECONDS_PER_YEAR * DECIMALS_ADJUSTMENT);
//         uint256 availableToWithdraw = account
...
//         return
//             (block.timestamp - _HLP_PORTAL.lastTradeTime(address(this))) >=
//             TRADE_TIMELOCK;
//     }
...
//             if (_ENERGY_TOKEN.balanceOf(address(msg.sender)) < remainingDebt)
//                 revert ErrorsLib.InsufficientPEtokens();
...
//             _ENERGY_TOKEN.safeTransferFrom(
//                 msg.sender,
//                 address(this),
//                 remainingDebt
//             );
...
//         if (!_checkLastTrade()) revert ErrorsLib.TradeTimelockActive();
//         Account memory account = _updateAccount(msg.sender, 0);
//         if (account.portalEnergy < _amount)
...
//         address _recipient,
//         uint256 _amount
//     ) external nonReentrant existingAccount(_recipient) {
//         if (_recipient == address(0)) revert ErrorsLib.InvalidInput();
//         if (_amount == 0) revert ErrorsLib.InvalidInput();
//         if (!_checkLastTrade()) revert ErrorsLib.TradeTimelockActive();

//         /// @dev Require that the caller has sufficient tokens to burn
//         if (_ENERGY_TOKEN.balanceOf(msg.sender) < _amount)
//             revert ErrorsLib.InsufficientBalance();
...
//         uint256 _deadline
//     ) external nonReentrant ensure(_deadline) existingAccount(_user) {
//         if (_amount == 0) revert ErrorsLib.InvalidInput();
//         if (_minReceived == 0) revert ErrorsLib.InvalidInput();
//         if (!_checkLastTrade()) revert ErrorsLib.TradeTimelockActive();
//         if (_PSM_TOKEN.balanceOf(msg.sender) < _amount)
//             revert ErrorsLib.InsufficientBalance();
...
//         emit EventsLib.PortalEnergyBuyExecuted(
//             msg.sender,
//             _user,
//             amountReceived
//         );
...
//             _swap.psmAmount
//         );
//         (, uint256 spentAmount_) = _ONE_INCH_V5_AGGREGATION_ROUTER_CONTRACT
//             .swap(_executor, _description, "", _data);

//         if (!_forLiquidity) {
//             uint256 remainAmount = _swap.psmAmount - spentAmount_;
//             if (remainAmount > 0)
//                 _PSM_TOKEN.safeTransfer(msg.sender, remainAmount);
//         }
//     }
...
//         emit EventsLib.PortalEnergySellExecuted(
//             msg.sender,
//             _receiver,
//             amountReceived
//         );
...
//         );
//         if (pair == address(0)) {
//             pair = _RAMSES_FACTORY.createPair(
//                 PSM_TOKEN_ADDRESS,
//                 WETH_ADDRESS,
//                 false
//             );
...
//                     revert ErrorsLib.InsufficientBamount();
...
//                     revert ErrorsLib.InvalidOutput();
//                 if (amountAOptimal < amountAMin)
//                     revert ErrorsLib.InsufficientAamount();
...
//         nonReentrant
//         ensure(_deadline)
//         returns (uint256 amountPSM, uint256 amountWETH)
//     {
//         /// @dev validated input arguments
//         if (_receiver == address(0)) revert ErrorsLib.InvalidInput();
...
//     {
//         return
//             _RAMSES_ROUTER.quoteAddLiquidity(
//                 PSM_TOKEN_ADDRESS,
//                 WETH_ADDRESS,
//                 false,
//                 _amountPSMDesired,
...
//         return
//             _RAMSES_ROUTER.quoteRemoveLiquidity(
//                 PSM_TOKEN_ADDRESS,
//                 WETH_ADDRESS,
//                 false,
//                 _liquidity
